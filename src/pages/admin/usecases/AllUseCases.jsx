...
const [showDelete, setShowDelete] = useState(false);
  ...
  const createPost = async (newPost) => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/usecases', options);
      const data = await response.json();
  ...
  const updatePost = async (id, newPost) => {
    try {
      ...
      console.log(newPost)
      const response = await fetch(`http://localhost:4000/api/usecases/${id}`, options);
      const data = await response.json();
  ...
  const handleDelete = async () => {
    try {
    const response = await fetch(`http://localhost:4000/api/usecases/${selectedItem._id}`,
    {
    method: 'DELETE',
    }
    );
  ...
  return (
    ...
    <div class="grid-container">
     ...
     <div class="main-title">
          <p class="font-weight-bold">USE CASES PAGE</p>
        </div>
       ...
       <Table striped bordered hover size="sm">
       ...
       
        {
          ...
          items.map((item)=>(
            ...
            <td style={{width:'20%'}}>
              {item.category==="6418367bbd2c7482288c4253"? 'Aerial and geospatial' :
              item.category==="64183be4bd2c7482288c42db"? 'Agriculture' : 
              item.category==="641729e69c8636ff40017d92"? 'Autonomous Driving' : 
              item.category==="64172a239c8636ff40017d9b"? 'Retail' : 
              item.category==="641729e69c8636ff40017d92"? 'Insurance' :  
              item.category==="641729e69c8636ff40017d92"? 'Finance' : "" 
